...
function initCheckBox(){
	...
	$(".add").click(function(){
		...
		$(this).parents('.m-num').siblings('.price').find('.shop_price').text(all)
...
function cart(id){
	...
	if(id==2||id==1){
		if(Alldark!=dark&&id==2){ //判断是否全选
			$(".dark-check,.total-check .check").addClass('action');
			$("#del").show();
		}else if(Alldark==dark&&id==2){ //全选了后去掉全选
			$(".dark-check,.total-check .check").removeClass('action');
			$("#del").hide();
		}else if(dark==0&&id==1){//判断单选没有一个选中
			$("#del").hide();
		}else if(Alldark==dark&&id==1){//如果单选按钮全亮就点亮全选按钮
			$("#del").show();
			$(".total-check .check").addClass('action');
		}else if(Alldark!=dark&&id==1){//如果单选按钮未全亮就取消全选按钮的点亮
			$("#del").show();
			$(".total-check .check").removeClass('action');
		}
		//获取list中的价格
		var list = [];
		$(".list ul li").each(function(item,ind){
			if($(this).find('.dark-check').hasClass('action')){
				list.push($(this).find('.price span:first-child').text());
			}
		})
